...
int main()
{
	...
	for (int k = 5; k < n + 3; ++k) {
		...
		dif[k][1] = Func(dif[k][0]);

		dif[k-1][2] = dif[k][1] - dif[k-1][1];
		dif[k-2][3] = dif[k-1][2] - dif[k-2][2];
		dif[k-3][4] = dif[k-2][3] - dif[k-3][3];
		dif[k-4][5] = dif[k-3][4] - dif[k-4][4];

	}
	for (int k = 5; k < n + 3; ++k) {
		cout << "k = " << k - 2 << ": knot: " << knot[k - 5] << "  value: " << dif[k][0] << endl;
		if (k == n + 2) {
			cout << "Абсолютная фактическая погрешность: " << fabs(func(knot[k - 5]) - dif[k][0]) << endl << endl;
		}
	}
